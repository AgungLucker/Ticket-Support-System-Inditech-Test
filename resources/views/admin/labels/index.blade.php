<x-app-layout>
    ...
    <div class="py-12" x-data="labelManager()" @open-modal.window="openAddModal()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <form method="GET" action="{{ route('admin.labels.index') }}" class="mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-3">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau slug label..." class="flex-1 rounded-xl border-gray-300">
                    <button class="bg-gray-800 hover:bg-gray-900 text-white px-4 py-2 rounded-xl">Cari</button>
                    <a href="{{ route('admin.labels.index') }}" class="bg-gray-100 hover:bg-gray-200 text-gray-800 px-4 py-2 rounded-xl text-center">Reset</a>
                </form>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-2xl border border-gray-100">
                    ...
                    <div class="p-4">{{ $labels->links() }}</div>
                </div>
            </div>
